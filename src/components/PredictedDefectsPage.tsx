  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
       <div className="flex items-center justify-between mb-2">
        <h2 className="text-xl font-semibold">Predicted Defects
</h2>
      </div>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <ul className="space-y-2">
            {Object.entries(dummyDefects).map(([defect, found], idx) => (
              <li key={idx} className="flex items-center gap-3">
                <span
                  className={`inline-block w-3 h-3 rounded-full ${found ? 'bg-red-500' : 'bg-green-500'}`}
                ></span>
                <span>{defect}</span>
              </li>
            ))}
          ...
          {onGoHome && (
            <button
              className="mt-8 w-full px-6 py-3 bg-green-600 text-white rounded hover:bg-green-700 text-lg font-semibold"
              onClick={onGoHome}
            >
